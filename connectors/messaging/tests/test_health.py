...
def test_health():
    ...
    assert body["service"] == "connector-messaging"


def test_catalogue_loaded():
    assert len(MY_CONNECTORS) == 8


def test_stub_dispatch():
    resp = client.post(
        "/connector/execute",
        json={"sender": "aepx://agent/x", "receiver": "aepx://connector/azure-servicebus", "payload": {"op": "ping"}},
    )
    assert resp.status_code == 200
    assert resp.json()["source"] == "connector:azure-servicebus"
